...
Axentix.Config = (() => {
  ...
  var register = (el, term) => {
    if (!el.name || !el.class) {
      console.error("Error registering ".concat(term, " : Missing required parameters."));
      return;
    }

    if (config[term].some(elem => elem.name === el.name)) {
      console.error("Error registering ".concat(term, " : Already exist."));
      return;
    }
...
Axentix.DataDetection = (() => {
  ...
  var getOptions = function getOptions(obj, component, element) {
...
(() => {
  ...
  class Caroulix extends AxentixComponent {
    ...
    _setup() {
      ...
      this.animFunction = '_animation' + this.options.animationType.charAt(0).toUpperCase() + this.options.animationType.substring(1);
...
(() => {
  ...
  class Sidenav extends AxentixComponent {
    ...
    _setup() {
      ...
      this.options.overlay ? this._createOverlay() : '';
      this.el.classList.contains('large') && this.layoutEl ? this.layoutEl.classList.add('sidenav-large') : this.layoutEl.classList.remove('sidenav-large');
...
Axentix.getInstance = element => {
  var el = Axentix.instances.find(ins => '#' + ins.instance.el.id === element);
